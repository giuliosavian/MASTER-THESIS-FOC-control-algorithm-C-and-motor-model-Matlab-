...
#define SPEEDUNIT2RADS(f)				(2*PI*(f)*6) / (60)
/* Current conversion from mA unit to 16Bit Digit */
#define CURRENT_CONV_FACTOR			(((65536.0 * RSHUNT * AMPLIFICATION_GAIN)/ADC_REFERENCE_VOLTAGE)/1000.0)
#define CURRENT_CONV_FACTOR_INV		((1000.0 * ADC_REFERENCE_VOLTAGE)/((65536.0 * RSHUNT * AMPLIFICATION_GAIN)))

#define REP_COUNTER 				(uint16_t) ((REGULATION_EXECUTION_RATE *2u)-1u)

#define INRUSH_CURRLIMIT_DELAY_COUNTS  (uint16_t)(INRUSH_CURRLIMIT_DELAY_MS * ((uint16_t)SPEED_LOOP_FREQUENCY_HZ)/1000u -1u)
