...
__weak bool HALL_CalcAvrgMecSpeedUnit(HALL_Handle_t *pHandle, int16_t *hMecSpeedUnit)
{
  ...
  else
  {
      ...
      else
      {
        ...
        else
        {
          ...
          if (pHandle->AvrElSpeedDpp != HALL_MAX_PSEUDO_SPEED)
          {
            ...
            mecSpeedUnit = (int16_t)((pHandle->AvrElSpeedDpp * (int32_t)pHandle->_Super.hMeasurementFrequency
                        * (int32_t)SPEED_UNIT )
                        / ((int32_t)(pHandle->_Super.DPPConvFactor) * (int32_t)(pHandle->_Super.bElToMecRatio)) );
          }
          else
          {
        	  mecSpeedUnit = (int16_t)pHandle->SatSpeed;
          }
        }
      }
    ...
    if(hMecSpeedUnit) *hMecSpeedUnit = mecSpeedUnit;
#ifdef NULL_PTR_CHECK_HALL_SPD_POS_FDB
  }
#endif
...
__weak void *HALL_TIMx_CC_IRQHandler(void *pHandleVoid)
{
  ...
  if (pHandle->SensorIsReliable)
  {
    ...
    uint32_t _h1 = LL_GPIO_IsInputPinSet(pHandle->H1Port, (1ul<<pHandle->H1Pin));
    uint32_t _h2 = LL_GPIO_IsInputPinSet(pHandle->H2Port, (1ul<<pHandle->H2Pin));
    uint32_t _h3 = LL_GPIO_IsInputPinSet(pHandle->H3Port, (1ul<<pHandle->H3Pin));
    ...
    {
      pHandle->HallState  = (uint8_t)( (_h3 << 2U)
                                      |(_h2 << 1U)
                                      |_h1);
    }
    else
    {
      pHandle->HallState  = (uint8_t)(((_h2 ^ 1U) << 2U)
                                      | (_h3 << 1U)
                                      | _h1);
    }
  ...
  HALL_CalcAvrgMecSpeedUnit(pHandle, NULL);
