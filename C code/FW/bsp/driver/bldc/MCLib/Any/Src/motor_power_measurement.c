...
/**
  ...
  *         expressed in watt.
  */
__weak int16_t MPM_GetElMotorPowerW(const MotorPowMeas_Handle_t *pHandle)
{
#ifdef NULL_PTR_CHECK_MOT_POW_MES
  return ((MC_NULL == pHandle) ? 0 : pHandle->hMeasBuffer[pHandle->hLastMeasBufferIndex]);
#else
...
/**
  ...
  */
__weak int16_t MPM_GetAvrgElMotorPowerW(const MotorPowMeas_Handle_t *pHandle)
{
#ifdef NULL_PTR_CHECK_MOT_POW_MES
