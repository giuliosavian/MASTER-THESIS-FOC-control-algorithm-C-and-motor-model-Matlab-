...
__weak uint16_t PWMC_SetPhaseVoltage(PWMC_Handle_t *pHandle, alphabeta_t Valfa_beta)
{
  ...
  else
  {
    ...
    g_dbg_pwmcurrfdbk.wX = wX;
    g_dbg_pwmcurrfdbk.wY = wY;
    g_dbg_pwmcurrfdbk.wZ = wZ;
    ...
    g_dbg_pwmcurrfdbk.PhA = (2*(float)pHandle->CntPhA / (float)pHandle->PWMperiod)*100;
    g_dbg_pwmcurrfdbk.PhB= (2*(float)pHandle->CntPhB / (float)pHandle->PWMperiod)*100;
    g_dbg_pwmcurrfdbk.PhC = (2*(float)pHandle->CntPhC / (float)pHandle->PWMperiod)*100;
